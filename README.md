...
## 비밀
`.env`(gitignore)에 토큰. `.env.example` 복사해 채운다. install이 OS 환경변수로 등록하고, config/MCP의 `{{OBSIDIAN_API_TOKEN}}`을 치환한다. github 토큰은 config가 env를 직접 참조(`${GITHUB_PERSONAL_ACCESS_TOKEN}`).
...
## 경로 템플릿
`{{PYTHON}}`(Codex 훅 인터프리터), `{{CODEX_HOME}}`, `{{VAULT}}`, `{{OBSIDIAN_API_TOKEN}}` →
`apply.py`가 OS·.env 기준으로 치환.
