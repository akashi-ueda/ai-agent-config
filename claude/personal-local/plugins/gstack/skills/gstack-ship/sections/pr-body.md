...
2. Compute the corrected title: `NEW_TITLE=$($HOME/.gstack/core/bin/gstack-pr-title-rewrite.sh "$NEW_VERSION" "$CURRENT")`. The helper handles three cases: title already correct (no-op), title has a different `v<X.Y.Z.W>` prefix (replace it), or title has no version prefix (prepend one).
...
<PR body from above>
PR_BODY_EOF
$HOME/.gstack/core/bin/gstack-redact --from-file "$PR_BODY_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json
case $? in
...
printf '%s' "v$NEW_VERSION <type>: <summary>" | $HOME/.gstack/core/bin/gstack-redact --repo-visibility "$REDACT_VIS" --json
```
