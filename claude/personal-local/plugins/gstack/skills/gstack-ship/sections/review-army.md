...
```bash
source <($HOME/.gstack/core/bin/gstack-diff-scope <base> 2>/dev/null)
```
...
```bash
$HOME/.gstack/core/bin/gstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
source <($HOME/.gstack/core/bin/gstack-diff-scope <base> 2>/dev/null) || true
...
```bash
$HOME/.gstack/core/bin/gstack-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `$HOME/.gstack/core/review/specialists/testing.md`
2. **Maintainability** — read `$HOME/.gstack/core/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `$HOME/.gstack/core/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `$HOME/.gstack/core/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `$HOME/.gstack/core/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `$HOME/.gstack/core/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `$HOME/.gstack/core/review/design-checklist.md`
...
```bash
$HOME/.gstack/core/bin/gstack-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `$HOME/.gstack/core/review/specialists/red-team.md`
...
```bash
$HOME/.gstack/core/bin/gstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
