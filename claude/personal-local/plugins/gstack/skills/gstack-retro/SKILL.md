...
preamble-tier: 2
version: 2.0.0
description: 주간 엔지니어링 회고 — 인별 분석·배포 추세·테스트 건전성. (gstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
$HOME/.gstack/core/bin/gstack-timeline-log '{"skill":"retro","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$HOME/.gstack/core/bin/gstack-question-log '{"skill":"retro","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
$HOME/.gstack/core/bin/gstack-learnings-log '{"skill":"retro","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
```bash
DISCOVER_BIN=""
[ -x $HOME/.gstack/core/bin/gstack-global-discover ] && DISCOVER_BIN=$HOME/.gstack/core/bin/gstack-global-discover
