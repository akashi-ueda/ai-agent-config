...
name: gstack-spec
version: 0.1.0
description: 모호한 의도를 5단계로 정밀 실행 스펙으로 전환. (gstack)
allowed-tools:
  - Bash
  - Read
  - Grep
...
$HOME/.gstack/core/bin/gstack-timeline-log '{"skill":"spec","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$HOME/.gstack/core/bin/gstack-question-log '{"skill":"spec","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
bun $HOME/.gstack/core/lib/redact-audit-log.ts \
...
REDACT_VIS=$($HOME/.gstack/core/bin/gstack-config get redact_repo_visibility 2>/dev/null)
...
REDACT_BODY_EOF
REDACT_JSON=$($HOME/.gstack/core/bin/gstack-redact --from-file "$REDACT_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json)
REDACT_CODE=$?
```
...
reuse it; write the exact bytes to `$REDACT_FILE`; `$HOME/.gstack/core/bin/gstack-redact --from-file "$REDACT_FILE"
...
eval "$($HOME/.gstack/core/bin/gstack-slug)"
