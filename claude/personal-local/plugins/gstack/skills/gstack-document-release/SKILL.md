...
preamble-tier: 2
version: 1.0.0
description: 출시 후 문서 갱신 — diff와 대조해 드리프트된 문서 업데이트. (gstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
$HOME/.gstack/core/bin/gstack-timeline-log '{"skill":"document-release","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$HOME/.gstack/core/bin/gstack-question-log '{"skill":"document-release","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
$HOME/.gstack/core/bin/gstack-redact --from-file /tmp/gstack-pr-body-$$.md --repo-visibility "${REDACT_VIS:-unknown}" --json
...
```bash
NEW_TITLE=$($HOME/.gstack/core/bin/gstack-pr-title-rewrite.sh "$V" "$CURRENT_TITLE")
```
