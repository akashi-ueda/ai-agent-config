...
- `graphify`는 현재 Claude plugin이 아니라 `C:\Users\ENITEC\.claude\skills\graphify`와 `graphifyy` CLI로 사용한다.
...
- `graphify`: 코드, 문서, 아키텍처 관계를 지식 그래프로 탐색.
...
- 발트: `obsidian` MCP로 접근(`OBSIDIAN_VAULT_PATH` 기준). 업무용 LLM 위키.
- 비단순 프로젝트/작업 질문이거나 저장 가치가 있으면, 먼저 발트 루트의 `AGENT-GUIDE.md`(행동+구조 통합 운영 규칙)를 읽고 따른다.
- vault 전체를 읽지 않는다. 요청 키워드로 `search_vault` 후 가장 관련 높은 1–3개 노트만 읽는다.
- 저장 가치 있는 결정·절차·설계 이유·재사용 교훈은 자동 저장하되, 기존 노트 중복을 먼저 검색한다.
- API 키·토큰·비밀번호·인증 헤더·세션 쿠키·개인정보·`.env`는 저장하지 않는다.
- 저장 후 경로와 검증 결과만 짧게 보고하고, 새 위키 페이지면 `index.md` 등록·`log.md` 기록을 따른다.
