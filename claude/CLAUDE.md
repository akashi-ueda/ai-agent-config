...
`attribution-reminder` 플러그인(skill + UserPromptSubmit 훅, github.com/akashi-ueda/attribution-reminder)이 담당한다. 자동 사용한 스킬/MCP/훅을 응답 마지막 한 줄로 공개하며, 매 턴 훅이 포맷을 주입한다. 언어는 `AGENT_ATTRIBUTION_LOCALE=ko`로 한국어 고정.
